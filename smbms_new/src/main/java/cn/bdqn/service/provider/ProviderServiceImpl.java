...
import cn.bdqn.dao.bill.BillDao;
import cn.bdqn.dao.bill.BillDaoImpl;
import cn.bdqn.dao.provider.ProviderDao;
import cn.bdqn.dao.provider.ProviderDaoImpl;
...
public class ProviderServiceImpl implements ProviderService {
	...
	public List<Provider> getPageProviderList(Provider provider,
		...
		List<Provider> pageProviderList = null;
		Connection connection = null;
		try {
			connection = BaseDao.getConnection();
			pageProviderList = providerDao.getPageProviderList(connection, provider, pageInfo);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			BaseDao.closeResource(connection, null, null);
	...
	public int getRecCountByProvider(Provider provider) {
		...
		try {
			connection = BaseDao.getConnection();
			providerCount = providerDao.getRecCountByProvider(connection, provider);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			BaseDao.closeResource(connection, null, null);
