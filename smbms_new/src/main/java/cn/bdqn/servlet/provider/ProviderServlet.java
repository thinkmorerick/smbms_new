package cn.bdqn.servlet.provider;
...
import cn.bdqn.service.bill.BillService;
import cn.bdqn.service.bill.BillServiceImpl;
import cn.bdqn.service.provider.ProviderService;
import cn.bdqn.service.provider.ProviderServiceImpl;
...
public class ProviderServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (method != null && method.equals("query")) {
			...
			this.getProviderById(request, response, "providerUpdate.jsp");
	...
	private void addpage(HttpServletRequest request,
		...
		request.getRequestDispatcher("WEB-INF/jsp/provider/providerAdd.jsp").forward(request,
				response);
	}
	...
	private void modify(HttpServletRequest request, HttpServletResponse response)
		...
		if (providerService.modify(provider)) {
			...
			request.getRequestDispatcher("WEB-INF/jsp/provider/providerUpdate.jsp").forward(
					request, response);
	...
	private void getProviderById(HttpServletRequest request,
		...
		if (!StringUtils.isNullOrEmpty(id)) {
			...
			request.getRequestDispatcher("/WEB-INF/jsp/provider/"+url).forward(request, response);
		}
	}
	...
	private void add(HttpServletRequest request, HttpServletResponse response)
		...
		provider.setCreateDate(new Timestamp(System.currentTimeMillis()));
	...
	private void query2(HttpServletRequest request, HttpServletResponse response)
		...
		request.getRequestDispatcher("WEB-INF/jsp/provider/providerList.jsp").forward(request,
				response);
	}
	...
	// 分页查询用户列表
		public void query(HttpServletRequest request, HttpServletResponse response)
			...
			int providerPageSize = Constants.PROVIDER_PAGE_SIZE;
			// 实例化PageSupport对象
			PageSupport pageSupport = new PageSupport();
			pageSupport.setPageSize(providerPageSize);
			// 查询总记录数
			int recCount = providerService.getRecCountByProvider(provider);
			...
			pageInfo.put(Constants.PAGE_SIZE, Integer.valueOf(providerPageSize));

			providerList = providerService.getPageProviderList(provider, pageInfo);
			request.setAttribute("pageSupport", pageSupport);
			// 设置providerlist的分页选项容量
